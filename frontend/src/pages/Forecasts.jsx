...
export default function Forecasts() {
  ...
  useEffect(() => {
    const fetchForecastData = async () => {
      try {
        setLoading(true);
        const region = filters.region || "East";
        const data = await api.getForecast7Days(region);
  ...
  const metrics = useMemo(() => {
    // If data is still loading or error, return fallback data
    if (!forecastData) {
    ...
    // Use real API data
    const cpuForecast = forecastData.predictions_cpu || forecastData.predictions || [];
    const storageForecast = forecastData.predictions_storage || [];

    const cpuCurrent = cpuForecast.length > 0 ? cpuForecast[0] : 70;
    const storageCurrent = storageForecast.length > 0 ? storageForecast[0] : 2.5;

    // Estimate network from CPU (simple heuristic)
    ...
    return [
      {
        ...
        pie: [
          { name: "Used", value: Math.min(100, Math.round(cpuCurrent)) },
          { name: "Remaining", value: Math.max(0, 100 - Math.round(cpuCurrent)) },
        ],
      },
      {
        id: "storage",
        ...
        pie: (() => {
          const storageNext = storageForecast.length > 0 ? storageForecast[storageForecast.length - 1] : storageCurrent;
          const usedPercent = Math.min(100, Math.round((storageCurrent / Math.max(1, storageNext)) * 100));
          return [
