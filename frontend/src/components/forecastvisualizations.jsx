...
export default function ForecastVisualizations() {
  ...
  // Run forecasts using real backend API
  const runForecasts = async () => {
    ...
    try {
      ...
      const data = len === 7
        ? await api.getForecast7Days(region)
        : await api.getForecast30Days(region);

      const cpuForecast = data.predictions_cpu || data.predictions || [];
      const storageForecast = data.predictions_storage || [];

      // Use the appropriate forecast based on metric selection
      const forecastValues = metric === "CPU Usage" ? cpuForecast : storageForecast;

      // Generate dates
      const baseDates = Array.from({ length: Math.min(len, forecastValues.length) }, (_, i) => `Day ${i + 1}`);

      // For "Historical" we'll use a simulated baseline (since we don't have actual historical data in API)
      // In production, you'd fetch historical data from another endpoint
      const baseValue = forecastValues[0] || 70;
      const newActual = baseDates.map((_, i) =>
        baseValue + (i * 0.5) + (Math.random() - 0.5) * 3
      );

      const newForecast = forecastValues.slice(0, len);
      const newLower = newForecast.map((v) => v * 0.92); // 8% lower bound
      const newUpper = newForecast.map((v) => v * 1.08); // 8% upper bound
      ...
      // Anomaly detection on actuals
      const n = newActual.length;
      if (n > 0) {
        const meanActual = newActual.reduce((s, v) => s + v, 0) / n;
      ...
      const metricBase = metric === "CPU Usage" ? 60 : 50;

      setDates(baseDates);
      setActual(baseDates.map((_, i) => metricBase + i * 2 + (i % 3) * 3));
      setForecast(baseDates.map((_, i) => metricBase + i * 2.5 + 5));
      setLower95(baseDates.map((_, i) => metricBase + i * 2.5 - 3));
      setUpper95(baseDates.map((_, i) => metricBase + i * 2.5 + 13));
      setAnomalies([]);
    }
  ...
  // Auto-run on mount and when region/metric/horizon changes
  useEffect(() => {
    runForecasts();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [region, metric, horizon]);
      ...
      <div className="rounded-2xl border border-gray-200/70 dark:border-gray-800 bg-white/70 dark:bg-slate-950/80 backdrop-blur-md shadow-xl p-5 md:p-6 space-y-8">
        ...
        <div className="flex flex-col gap-4 pb-4 mb-4 border-b border-gray-200 dark:border-gray-800">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              {["random_forest", "arima", "xgboost", "lstm"].map((m) => {
                const isActive = model === m;
                return (
                  <button
                    key={m}
                    onClick={() => setModel(m)}
                    className={`${filterPill} capitalize ${isActive
                  ...
                  >
                    {m === "random_forest" ? "Random Forest (Active)" : m}
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex flex-wrap gap-2 mb-2 text-xs">
            {anomalies.map((a) => (
              <button
                ...
                className={`px-3 py-1 rounded-full border transition ${selectedAnomaly?.date === a.date
