...
function SystemUsageTable({ dashboardData }) {
  ...
  const cpuUsageData = (() => {
    if (!dashboardData) {
      return labels.map(() => getRandomPercentage(30, 100));
    }
    const cpuForecast = dashboardData.predictions_cpu || [];
    return labels.map((_, idx) =>
      cpuForecast[idx] ? Math.round(cpuForecast[idx]) : getRandomPercentage(30, 100)
    );
  })();

  const storageUsageData = (() => {
    if (!dashboardData) {
      return labels.map(() => getRandomPercentage(40, 100));
    }
    const storageForecast = dashboardData.predictions_storage || [];
    return labels.map((_, idx) =>
      storageForecast[idx] ? storageForecast[idx].toFixed(1) : getRandomPercentage(40, 100)
    );
  })();
        ...
        <table className="min-w-full border border-[#b7d2f7] dark:border-fuchsia-700 text-sm text-[#284266] dark:text-orange-100">
          <thead className="bg-[#ebedf0] dark:bg-gray-800 text-[#405060] dark:text-orange-50 uppercase text-sm">
            <tr>
              ...
              <th className="py-3 px-4 border">Storage Usage (TB)</th>
            </tr>
          </thead>
          <tbody>
...
function DashboardLayout() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        const data = await api.getForecast7Days();
  ...
  // Generate KPI data from API or fallback to random
  const kpiData = (() => {
    if (!dashboardData) {
      // Fallback to random data while loading
      return [
        {
          title: "CPU Usage",
    ...
    // Use real API data
    const cpuForecast = dashboardData.predictions_cpu || dashboardData.predictions || [];
    const storageForecast = dashboardData.predictions_storage || [];

    const currentCPU = cpuForecast.length > 0 ? Math.round(cpuForecast[0]) : 70;
    const currentStorage = storageForecast.length > 0 ? storageForecast[0].toFixed(1) : "12.5";

    // Calculate delta (compare first vs last prediction)
    const cpuDelta = cpuForecast.length > 1
      ? Math.round(cpuForecast[cpuForecast.length - 1] - cpuForecast[0])
      : 0;
    const storageDelta = storageForecast.length > 1
      ? (storageForecast[storageForecast.length - 1] - storageForecast[0]).toFixed(1)
      : 0;
    ...
    return [
      ...
      {
        title: "Storage Usage",
        value: `${currentStorage} TB`,
        delta: `${storageDelta > 0 ? '+' : ''}${storageDelta}`,
        subtitle: "7-day trend",
      },
      {
        title: "Server Uptime",
        value: `${(99 + Math.random()).toFixed(2)}%`,
        delta: `+0.${getRandomInt(1, 5)}`,
        subtitle: "Last 24 hours",
      },
    ];
  })();
  ...
  // Bar Chart: Current vs Forecast (using real data)
  const barChartData = (() => {
    if (!dashboardData) {
      return days.map((day) => ({
        name: day,
    ...
    const cpuForecast = dashboardData.predictions_cpu || [];
    const current = cpuForecast[0] || 70;

    return days.map((day, idx) => ({
      name: day,
      current: idx === 0 ? Math.round(current) : Math.round(current + (idx * 0.5)),
      forecast: cpuForecast[idx] ? Math.round(cpuForecast[idx]) : Math.round(current + (idx * 2)),
    }));
  })();

  // Pie Chart: Keep as traffic demo (not related to forecasting)
  const pieChartData = [
  ...
  // Line Chart: 7-day CPU trend (using real data)
  const lineChartData = (() => {
    if (!dashboardData) {
      return days.map((day) => ({
        name: day,
    ...
    return days.map((day, idx) => ({
      name: day,
      value: cpuForecast[idx] ? Math.round(cpuForecast[idx]) : getRandomInt(30, 100),
    }));
  })();
        ...
        return (
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              ...
              <div className="bg-[#f7f7f5] dark:bg-gradient-to-br dark:from-fuchsia-600 dark:to-orange-400 border border-[#b7d2f7] dark:border-none rounded-xl shadow px-6 py-6">
                <SystemUsageTable dashboardData={dashboardData} />
              </div>
            </div>
          </>
        );
